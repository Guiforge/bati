...
import type { SessionSummary } from "@/src/db";
import { getQuestSessionHistory, getRecentSessionHistory } from "@/src/db";
...
export function ProgressionChart({ questId, limit = 10, title }: ProgressionChartProps) {
  ...
  const chartData: ChartDataPoint[] = sessions.map((session) => {
    ...
    let barColor = "$primary"; // primary - medium
    if (session.userLevel === "easy") {
      barColor = "$success"; // success - easy
    } else if (session.userLevel === "hard") {
      barColor = "$error"; // error - hard
    }
