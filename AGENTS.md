...
- **Seed content and hero content share one table.** `exercises` holds both, told apart by
  ...
  price is one rule: every migration statement that `UPDATE`s or `DELETE`s `exercises` scopes
  itself to `creator`. `__tests__/seed-migration-guard.test.ts` fails on any that does not, and
