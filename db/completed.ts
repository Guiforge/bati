 ...
 */
export function parseRecords(json: string | null): StoredRecord[] {
  if (!json) return [];
  try {
...
export async function deleteSession(sessionId: number): Promise<"deleted" | "locked"> {
  const { adventureRuns, adventureRunSteps, bossDamageLog, bossFights, gpsPoints } = schema;
  ...
  const outcome = await transactionOrFallback(async (tx) => {
    ...
    if (step) {
      ...
      const [newer] = await tx
        ...
        .from(adventureRuns)
        .where(
          and(eq(adventureRuns.adventureId, step.adventureId), gt(adventureRuns.id, step.runId)),
        );
      if ((later?.n ?? 0) > 0 || (newer?.n ?? 0) > 0) return "locked" as const;
