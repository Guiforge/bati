---
title: Coach & Planning System
type: system
status: deferred
updated: 2026-07-18
related: [../planning/roadmap-alignment.md, ../economy/rewards-and-progression.md]
sources: [db/goals.ts, db/plans.ts, db/scheduling.ts, db/restSuggestions.ts]
---

# Coach & Planning System

> ⚠️ **Status: deferred, not MVP.** Per
> [roadmap-alignment.md](../planning/roadmap-alignment.md) (cleanup item `C5`), Goals /
> Planning / Scheduling are removed from the MVP navigation surface. The source-of-truth
> product wiki (`bati.md`) describes the Coach as "Gardien de la Flamme": a **v1 with
> simple, explicit rules**, not an AI system — and explicitly notes it needs validation by
> real sports professionals before shipping any auto-generated training advice. Everything
> below documents the **full/future design**; the underlying code (`db/goals.ts`,
> `db/plans.ts`, `db/scheduling.ts`, `db/restSuggestions.ts`) is kept but not wired into the
> MVP loop.
