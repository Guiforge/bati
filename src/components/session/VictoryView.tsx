...
import { NarrativeModal } from "@/src/components/adventures/NarrativeModal";
import { AppButton } from "@/src/components/common/AppButton";
import { Card } from "@/src/components/common/Card";
import { useToast } from "@/src/components/common/Toast";
import { ConstructionAnimation } from "@/src/components/village/ConstructionAnimation";
import { getQuestColorTokensFromQuest } from "@/src/constants/exerciseColors";
import { SOUNDS } from "@/src/constants/sounds";
import { getAdventureStepOutroNarrative } from "@/src/db/adventures-narrative";
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (campaign?.nextQuestId && campaign.nextRunStepId) {
        router.replace(
          `/(modals)/quests/${campaign.nextQuestId}?level=${encodeURIComponent(userLevel)}&runStepId=${campaign.nextRunStepId}` as never
        );
        return;
      }

      if (campaign?.isFinished) {
        router.replace(`/(modals)/adventures/${campaign.adventureId}` as never);
        return;
      }
