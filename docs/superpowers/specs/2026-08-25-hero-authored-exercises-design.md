...
3. **Name lookups are official by default.** One `officialByName(enName)` used by the warm-up,
   the paths and the oath presets, instead of three copies of `find(e => e.enName === name)` that
   could each resolve to the hero's row.
...
`SCHEMA_VERSION` in [`db/schemaVersion.ts`](../../../db/schemaVersion.ts) bumps, because a backup
taken by this build must not be adopted by an older one.
...
/** Seed content only — the warm-up, the paths and the oaths resolve names through this. */
export function officialByName(enName: string): Promise<Exercise | null>;
